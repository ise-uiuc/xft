...
_CONFIG_FOR_DOC = "LlamaMoEUpscalingConfig"
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaMoEUpscalingConfig):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
...
class TopKUniversalBalancedNoNormalizationNoisyGate(nn.Module):
    def __init__(
        self,
        input_size,
        num_experts,
        ...
        super(TopKUniversalBalancedNoNormalizationNoisyGate, self).__init__()
        assert num_selects <= num_experts
        self.input_size = input_size
        self.num_experts = num_experts - 1
        self.num_selects = num_selects - 1

        self.gate_network_type = gate_network
        self.gate_network = self.get_gate_network(gate_network, self.input_size, self.num_experts)
        ...
        self.warned = False
        if self.add_noise:
            self.weight_noise = nn.Linear(self.input_size, self.num_experts, bias=False)
    ...
    def forward(self, x):
        ...
        all_logits, all_indices = logits.topk(
            self.num_experts, dim=1
        )
        top_k_1_logits = top_logits[:, : self.num_selects]
        top_k_1_indices = top_indices[:, : self.num_selects]
        top_k_1_scores = (
            self.softmax(all_logits.to(torch.float32))
            if self.use_softmax
            else all_logits
        )
        top_k_1_scores = top_k_1_scores[:, : self.num_selects].to(logits.dtype)

        zeros = torch.zeros_like(logits, requires_grad=True, device=logits.device)
        scores_filtered = zeros.scatter(
            dim=1, index=top_k_1_indices, src=top_k_1_scores
        )  # shape(batch_size, num_experts-1)
        importance = scores_filtered.sum(0)  # shape(num_experts-1)
        ...
        batch_size = top_logits.size(0)
        universal_indices = torch.zeros(batch_size, device=x.device).to(top_k_1_indices.dtype)
        top_k_indices = torch.cat((torch.add(top_k_1_indices, 1), universal_indices.unsqueeze(1)), dim=-1)

        # print("indices:", top_k_1_indices[0])
        # print("indices:", top_k_indices[0])

        universal_scores = torch.sub(torch.ones(batch_size, device=x.device).to(top_k_1_scores.dtype), top_k_1_scores[:,0])
        top_k_1_scores_softmax = (
            top_k_1_scores.to(torch.float32)
        )
        top_k_scores = torch.cat((top_k_1_scores_softmax, universal_scores.unsqueeze(1)), dim=-1)
        top_k_scores = top_k_scores.to(logits.dtype)

        # print("scores:", top_k_1_scores[0])
        # print("scores:", top_k_scores[0])
...
class TopKUniversalBalancedNoisyGate(nn.Module):
    def __init__(
        self,
        input_size,
        num_experts,
        ...
        super(TopKUniversalBalancedNoisyGate, self).__init__()
        assert num_selects <= num_experts
        self.input_size = input_size
        self.num_experts = num_experts - 1
        self.num_selects = num_selects - 1
    ...
    def forward(self, x):
        ...
        # print("indices:", top_k_1_indices.tolist())
        # print("*"*10)
        # print("indices:", top_k_indices[0])

        universal_scores = torch.sub(torch.ones(batch_size, device=x.device).to(top_k_1_scores.dtype), top_k_1_scores[:,0])
        top_k_1_scores_softmax = (
            self.softmax(top_k_1_scores.to(torch.float32))*top_k_1_scores[:,:1]
            if self.use_softmax
            else top_k_1_scores
        )
        top_k_scores = torch.cat((top_k_1_scores_softmax, universal_scores.unsqueeze(1)), dim=-1)
...
class LinearGLUExperts(nn.Module):
    ...
    def __init__(
        ...
        for i in range(num_experts):
            this_expert_gate_proj = nn.Linear(self.in_features, self.hidden_features, bias=False)
            this_expert_up_proj = nn.Linear(self.in_features, self.hidden_features, bias=False)
            this_expert_down_proj = nn.Linear(self.hidden_features, self.out_features, bias=False)
            self.weight_gate.append(this_expert_gate_proj)
            self.weight_up.append(this_expert_up_proj)
            self.weight_down.append(this_expert_down_proj)
    ...
    def forward(self, input, i):
        down_proj = self.weight_down[i](self.act_fn(self.weight_gate[i](input)) * self.weight_up[i](input))
        return down_proj
...
class BaseMoELayer(nn.Module):
    def __init__(self):
        ...
        self.gate: TopKBalancedNoisyGate | TopKUniversalBalancedNoisyGate | TopKUniversalBalancedNoNormalizationNoisyGate
        self.calculator: UniversalCalculator

    def _create_gate(self, layer_index, **kwargs):
        ...
        elif self.gate_type == "TopKUniversalBalancedNoisyGate":
            self.gate = TopKUniversalBalancedNoisyGate(
                self.input_size,
                self.num_experts,
                self.num_selects,
                gate_network=kwargs.get("gate_network", "mlp"),
        ...
        elif self.gate_type == "TopKUniversalBalancedNoNormalizationNoisyGate":
            self.gate = TopKUniversalBalancedNoNormalizationNoisyGate(
                self.input_size,
                self.num_experts,
                self.num_selects,
                gate_network=kwargs.get("gate_network", "mlp"),
...
class LinearGLUMoELayer(BaseMoELayer):
    def __init__(
        ...
        **kwargs,
        ...
        self._create_gate(self.layer_index, **kwargs)
        self._create_calculator(experts, **kwargs)


class LinearUniversalGLUMoELayer(BaseMoELayer):
    def __init__(
        self,
        input_size,
        hidden_size,
        ...
        **kwargs,
        ...
        super(LinearUniversalGLUMoELayer, self).__init__()
        assert num_selects <= num_experts
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.output_size = output_size
        ...
        self.weight_gate_universal = nn.Linear(input_size, hidden_size, bias=False)
        self.weight_up_universal = nn.Linear(input_size, hidden_size, bias=False)
        self.weight_down_universal = nn.Linear(hidden_size, output_size, bias=False)
...
class LlamaMoEUpscalingDecoderLayer(nn.Module):
    def __init__(self, config: LlamaMoEUpscalingConfig, layer_index):
        super().__init__()
...
class LlamaMoEUpscalingPreTrainedModel(PreTrainedModel):
    config_class = LlamaMoEUpscalingConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["LlamaMoEUpscalingDecoderLayer"]
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, LlamaMoEUpscalingModel):
            module.gradient_checkpointing = value


class LlamaMoEUpscalingModel(LlamaMoEUpscalingPreTrainedModel):
    def __init__(self, config: LlamaMoEUpscalingConfig):
        ...
        self.layers = nn.ModuleList(
            [LlamaMoEUpscalingDecoderLayer(config, i) for i in range(config.num_hidden_layers)]
        )
...
class LlamaMoEUpscalingForCausalLM(LlamaMoEUpscalingPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = LlamaMoEUpscalingModel(config)
        self.pretraining_tp = config.pretraining_tp
        self.vocab_size = config.vocab_size
