...
class Args:
    ...
    use_weighted: bool = field(default=False)
...
def train():
    ...
    data_collator = get_data_collator(args, tokenization_context.pad_token_id)

    if not args.use_moe and not args.use_weighted:
        ...
        model = LlamaMoEUpscalingForCausalLM.from_pretrained(
            model_name_or_path,
            torch_dtype = dtype,
        )
        print(model)
    ...
    elif args.use_weighted:
        model = LlamaWeightedForCausalLM.from_pretrained(
            model_name_or_path,
        )
        for name, param in model.named_parameters():
            if "average_weights" not in name:
    ...
    tokenization_context.tokenizer.save_pretrained(training_args.output_dir)
